...
module.exports = grammar(GO, {
  ...
  rules: {
    _top_level_declaration: ($, original) =>
      choice(
        original,
    ...
    expression: ($) => seq("{", optional($._expression), "}"),
    ...
    component_block: ($) => seq("{", repeat($._component_node), "}"),
    _component_node: ($) =>
      choice(
        $.element,
        $.style_element,
      ...
      seq(
        ...
        optional(
          seq(
            ...
            field(
              "alternative",
              choice($.component_block, $.component_if_statement),
            ),
          ),
        ),
      ),
    ...
    component_switch_default_case: ($) =>
      prec.right(seq("default", ":", optional($._component_node))),
    ...
    // Extend the grammar to include method calls and chains
    component_import: ($) =>
      prec.right(
        seq(
          "@",
          ...
          repeat($.method_chain), // Handle method chains after the initial component
          optional(field("body", $.component_block)),
        ),
      ),

    // Define a method chain as a sequence of one or more method calls
    method_chain: ($) =>
      seq(
        ".",
        field("name", $._component_identifier),
        optional(field("arguments", $.argument_list)), // Optional arguments for methods
      ),
    ...
    component_render: ($) => seq("{!", field("expression", $._expression), "}"),
    ...
    //
    // See https://templ.guide/syntax-and-usage/template-composition
    component_children_expression: ($) => seq("{", "children...", "}"),

    // a templ element, which is basically a HTML node

    element: ($) =>
      choice(
        seq($.tag_start, repeat($._component_node), $.tag_end),
        $.self_closing_tag,
        $.doctype,
      ),
    tag_start: ($) =>
      seq("<", field("name", $.element_identifier), repeat($._attribute), ">"),
    tag_end: ($) => seq("</", field("name", $.element_identifier), ">"),
    self_closing_tag: ($) =>
      seq("<", field("name", $.element_identifier), repeat($._attribute), "/>"),

    doctype: ($) => seq("<!", field("name", $.element_identifier), "html", ">"),
    ...
    style_element: ($) =>
      choice(
        ...
        seq("<", field("name", "style"), repeat($.attribute), "/>"),
      ),
    ...
    conditional_attribute_block: ($) =>
      seq("{", "\n", repeat(choice($.attribute, $.spread_attributes)), "}"),
    ...
    _css_block: ($) => seq("{", repeat($.css_property), "}"),
    css_property: ($) =>
      seq(
        field("name", $.css_property_name),
        ":",
        field("value", choice($.expression, $.css_property_value)),
        ";",
      ),
    ...
    dynamic_class_attribute_value: ($) =>
      prec(
        -1,
        seq("{", commaSep(choice($._string_literal, $._expression)), "}"),
      ),
    ...
    script_block: ($) => seq("{", optional($.script_block_text), "}"),
    ...
    script_element: ($) =>
      choice(
        ...
        seq("<", field("name", "script"), repeat($.attribute), "/>"),
      ),
